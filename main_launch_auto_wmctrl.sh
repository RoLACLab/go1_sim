...
# Function to set Gazebo window always on top
set_gazebo_on_top() {
    echo "Setting Gazebo window to always on top..."
    # Wait a moment for Gazebo window to appear
    sleep 3
    
    # Find and set Gazebo window to always on top
    wmctrl -r "Gazebo" -b add,above 2>/dev/null || echo "Could not set Gazebo window on top (wmctrl method)"
    
    # Alternative method using xdotool (backup)
    if command -v xdotool >/dev/null 2>&1; then
        GAZEBO_WINDOW=$(xdotool search --name "Gazebo" | head -1)
        if [ ! -z "$GAZEBO_WINDOW" ]; then
            xdotool windowfocus $GAZEBO_WINDOW
            wmctrl -i -r $GAZEBO_WINDOW -b add,above 2>/dev/null
        fi
    fi
}
...
run_iteration() {
    ...
    # Set display and window manager hints for always on top
    ...
    # Set Gazebo window to always on top
    set_gazebo_on_top
